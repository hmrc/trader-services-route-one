  ...
  def checkEachIfSome[T, E](
    ...
    (entity: T) =>
      extract(entity)
        ...
        .getOrElse(
          if (isValidIfNone) Valid(()) else Invalid(List("Some sequence expected but got None"))
        )
  ...
  def checkIfOnlyOneSetIsDefined[T](
    ...
    (entity: T) => {
      ...
      if (definedSetCount == 0)
        Invalid(
          List(
            s"At least one of an alternative property sets $expectations must be defined or true"
          )
        )
      else if (definedSetCount > 1)
        Invalid(
          List(s"Only one of an alternative property sets $expectations can be defined or true")
        )
      else Valid(())
    }
